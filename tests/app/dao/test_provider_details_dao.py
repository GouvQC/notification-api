...
@pytest.mark.parametrize('provider_identifier', ['sinch', 'sns'])
...
def test_dao_get_provider_stats(notify_db_session):
    ...
    assert result[1].identifier == 'sinch'
    assert result[1].notification_type == 'sms'
    assert result[1].supports_international is False
    assert result[1].active is True
    assert result[1].current_month_billable_sms == 0

    assert result[2].identifier == 'sns'
    assert result[2].display_name == 'AWS SNS'
    ...
    assert result[6].identifier == 'pinpoint'
    assert result[6].notification_type == 'sms'
    assert result[6].supports_international is False
    assert result[6].active is True
    assert result[6].current_month_billable_sms == 0
